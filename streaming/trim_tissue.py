...
def create_parser():
    ...
    parser.add_argument('--leave_empty_regions', action='store_true')
...
class TissueExtractor(object):
    ...
    def __init__(self, save_dir: str, masks_dir: str, mask_suffix: str, output_spacing=1.0, threshold_downsample=32, remove_empty_regions=True):
        ...
        self.masks_dir = Path(masks_dir) if masks_dir else None  # type:ignore
    ...
    def save_sampling_mask(self, mask, fname):
        mask_fname = Path(fname.parent, fname.stem + '_msk')
...
def extract_tissue(file, save_dir, masks_dir, mask_suffix, output_spacing, remove_empty_regions):
    extractor = TissueExtractor(
        save_dir=save_dir,
        masks_dir=masks_dir,
        mask_suffix=mask_suffix,
        output_spacing=output_spacing,
        remove_empty_regions=remove_empty_regions,
    )

    new_fname = save_dir / file.stem
    new_fname = new_fname.with_suffix('.jpg')

    extractor.extract_file(file, new_fname)
...
if __name__ == '__main__':
    ...
    if args.image:
        extract_tissue(args.image, args.save_dir, args.masks_dir, args.mask_suffix, args.output_spacing, not args.leave_empty_regions)
    else:
        images_dir = Path(args.slides_dir) if args.slides_dir else None
        if args.csv:
            import pandas as pd
            images = []
            df = pd.read_csv(args.csv)
            images = df.slide_id.apply(lambda s: Path(images_dir, f'{s}.{args.filetype}'))
        else:
            images = list(images_dir.glob('*.' + args.filetype))

        save_dir = Path(args.save_dir)
        save_dir.mkdir(exist_ok=True)
        Parallel(n_jobs=num_cores)(delayed(extract_tissue)(
            img,
            save_dir = save_dir,
            masks_dir = args.masks_dir,
            mask_suffix = args.mask_suffix,
            output_spacing = args.output_spacing,
            remove_empty_regions = not args.leave_empty_regions,
        ) for img in images)
